...
# ============================================================
# 23. HEATMAPS DE CORRELACAO - JANELA 2018-2026
# ============================================================
#
# OBJETIVO:
#   Heatmap 1:
#     - Meta Selic
#     - NPL 90 dias - Total Itau
#     - % da populacao adulta inadimplente - Serasa
#     - Inadimplencia PF Total - BCB
#
#   Heatmap 2:
#     - Meta Selic
#     - 8 modalidades de inadimplencia PF - BCB
#     - 5 taxas NPL 90 dias do Itau
#
# FREQUENCIA:
#   Todas as series sao harmonizadas para TRIMESTRAL.
#
#   * Meta Selic:
#       media da Meta Selic diaria dentro do trimestre.
#
#   * Serasa e series mensais do BCB:
#       valor do ULTIMO MES DISPONIVEL do trimestre.
#       Isso e mais coerente com o NPL do Itau, que e uma taxa
#       observada na data de fechamento do trimestre.
#
#   * Itau:
#       valor trimestral reportado.
#
# JANELA:
#   1T2018 a 2T2026.
#
# HISTORICO ITAU:
#   A planilha descontinuada usa a aba "NPL_Nova segmentação".
#   A planilha atual usa "NPL_com_TVM".
#
#   Como as duas possuem 4T2024, a PLANILHA NOVA tem prioridade
#   a partir de 4T2024. Assim:
#       antiga: 1T2018 a 3T2024
#       nova:   4T2024 em diante
#
# IMPORTANTE:
#   As series:
#     Inadimplencia_Nao_Consignado_Com_Garantia
#     Inadimplencia_Nao_Consignado_Sem_Garantia
#   so possuem dados a partir de nov/2025 no arquivo fornecido.
#   Em frequencia trimestral, isso gera apenas 3 observacoes
#   (4T25, 1T26 e 2T26).
#
#   Para evitar correlacoes enganosas, o heatmap 2 deixa como NA
#   correlacoes com menos de 5 pares de observacoes.
#   A matriz BRUTA e salva separadamente.
# ============================================================


# ------------------------------------------------------------
# 23.1 Caminho da planilha historica descontinuada do Itau
# ------------------------------------------------------------

path_itau_descontinuado <- localizar_arquivo(
  pasta_base,
  c(
    "Planilha de Séries Históricas - (Descontinuada).xlsx",
    "Planilha de Séries Históricas - (Descontinuada)(1).xlsx"
  )
)


# ------------------------------------------------------------
# 23.2 Janela temporal
# ------------------------------------------------------------

inicio_heatmap <- as.Date("2018-01-01")
fim_heatmap <- as.Date("2026-06-30")

# Chave trimestral usada nos merges:
# 2018-01-01 = 1T2018, 2018-04-01 = 2T2018 etc.


# ------------------------------------------------------------
# 23.3 Funcoes auxiliares especificas
# ------------------------------------------------------------
...
# Extrai uma taxa NPL de uma linha fixa da planilha do Itau.
# Os valores do Excel estao em proporcao:
# 0,0195 = 1,95%.
# Portanto, multiplicamos por 100.

extrair_npl_itau_heatmap <- function(
    path,
    aba,
    linha,
    nome_saida) {
  ...
  # Datas estao na linha 2, a partir da coluna B.
  datas <- parse_data_itau(
    unlist(
      dados[2, -1],
      use.names = FALSE
...
# Calcula matriz de Pearson e, separadamente,
# numero de pares usados em cada celula.

calcular_correlacao_com_n <- function(
    base,
  ...
  matriz_cor_filtrada <- matriz_cor_bruta
  ...
  list(
    cor_bruta = matriz_cor_bruta,
    cor_filtrada = matriz_cor_filtrada,
    n = matriz_n
  )
}


# Converte uma matriz para data.frame preservando os nomes.
...
# Heatmap:
# - fill = correlacao;
# - texto = correlacao e numero de pares;
# - correlacoes com n < minimo_n aparecem em cinza.
...
criar_heatmap_correlacao <- function(
  ...
  cor_long <- resultado_cor$cor_bruta %>%
    as.data.frame() %>%
    tibble::rownames_to_column(
      "Variavel_1"
    ) %>%
  ...
  dados_heatmap <- cor_long %>%
    ...
    mutate(
      ...
      Variavel_1_Label = dplyr::recode(
        Variavel_1,
        !!!nomes_exibicao
      ),
      
      Variavel_2_Label = dplyr::recode(
        Variavel_2,
        !!!nomes_exibicao
      )
    )
  
  # Mantem a mesma ordem das colunas da matriz.
  ordem <- unname(
    nomes_exibicao[
      colnames(
        resultado_cor$cor_bruta
      )
    ]
  )
  ...
  dados_heatmap <- dados_heatmap %>%
    mutate(
      Variavel_1_Label = factor(
        Variavel_1_Label,
        levels = ordem
      ),
      Variavel_2_Label = factor(
        Variavel_2_Label,
        levels = rev(ordem)
      )
    )
...
# ------------------------------------------------------------
# 23.4 Meta Selic trimestral
# ------------------------------------------------------------
#
# Media da Meta Selic diaria dentro do trimestre.

selic_heatmap_trimestral <- selic_diaria %>%
  filter(
    Data >= inicio_heatmap,
    Data <= fim_heatmap
  ) %>%
...
# ------------------------------------------------------------
# 23.5 Serasa - % da populacao adulta inadimplente
# ------------------------------------------------------------
#
# Para cada trimestre, usa a ultima observacao mensal disponivel.

serasa_heatmap_trimestral <- serasa %>%
  filter(
    Data >= inicio_heatmap,
    Data <= fim_heatmap
  ) %>%
  ...
  summarise(
    Serasa_Populacao_Adulta_pct = ultimo_nao_na(
      Serasa_Populacao_Adulta_pct
    ),
    .groups = "drop"
  )
...
# ------------------------------------------------------------
# 23.6 Inadimplencia PF Total - arquivo SGS
# ------------------------------------------------------------
#
# Esta e a mesma serie BCB 21084 que aparece como
# Inadimplencia_PF_Total_modalidades no arquivo de modalidades.
#
# Para o primeiro heatmap, a serie e lida do arquivo SGS indicado
# pelo usuario e renomeada para manter a nomenclatura solicitada.

inad_sgs_heatmap_trimestral <- inad_sgs %>%
  filter(
    Data >= inicio_heatmap,
    Data <= fim_heatmap
  ) %>%
...
# ------------------------------------------------------------
# 23.7 Modalidades de inadimplencia - trimestral
# ------------------------------------------------------------

modalidades_heatmap_trimestral <- modalidades %>%
  filter(
    Data >= inicio_heatmap,
    Data <= fim_heatmap
  ) %>%
...
# ------------------------------------------------------------
# 23.8 NPL 90 dias Itau - historico antigo + planilha atual
# ------------------------------------------------------------

abas_itau_antigo <- readxl::excel_sheets(
  path_itau_descontinuado
)

aba_npl_itau_antigo <- dplyr::case_when(
  "NPL_com_TVM" %in% abas_itau_antigo ~ "NPL_com_TVM",
  "NPL_Nova segmentação" %in% abas_itau_antigo ~ "NPL_Nova segmentação",
  TRUE ~ NA_character_
)

if (is.na(aba_npl_itau_antigo)) {
  stop(
    paste0(
      "Nao encontrei no arquivo descontinuado as abas ",
      "'NPL_com_TVM' nem 'NPL_Nova segmentação'."
    )
  )
}


config_npl_heatmap <- tibble::tribble(
  ~Linha, ~Serie,
...
# 4T2024 comeca em 01/10/2024.
# A planilha NOVA substitui a antiga a partir desse trimestre.

corte_itau_novo <- as.Date(
  "2024-10-01"
)


lista_npl_heatmap <- purrr::map2(
  config_npl_heatmap$Linha,
  config_npl_heatmap$Serie,
  function(linha_atual, serie_atual) {
    
    historico_antigo <- extrair_npl_itau_heatmap(
      path = path_itau_descontinuado,
      aba = aba_npl_itau_antigo,
      linha = linha_atual,
      nome_saida = serie_atual
    ) %>%
      filter(
        Data_Trimestre < corte_itau_novo
      )
    
    historico_novo <- extrair_npl_itau_heatmap(
      path = path_itau,
      aba = "NPL_com_TVM",
      linha = linha_atual,
      nome_saida = serie_atual
    ...
    bind_rows(
      ...
      filter(
        Data_Trimestre >= inicio_heatmap,
        Data_Trimestre <= fim_heatmap
      ) %>%
      arrange(
        Data_Trimestre
      ) %>%
...
# ------------------------------------------------------------
# 23.9 HEATMAP 1
# Selic + NPL Total Itau + Serasa + Inadimplencia PF BCB
# ------------------------------------------------------------

base_heatmap_1 <- purrr::reduce(
  list(
    selic_heatmap_trimestral,
    ...
    serasa_heatmap_trimestral,
    
    inad_sgs_heatmap_trimestral
  ),
  inner_join,
  by = "Data_Trimestre"
) %>%
...
resultado_heatmap_1 <- calcular_correlacao_com_n(
  base = base_heatmap_1,
  minimo_n = 5
)
...
criar_heatmap_correlacao(
  resultado_cor = resultado_heatmap_1,
  nomes_exibicao = nomes_heatmap_1,
  titulo = "Correlacao entre Selic e indicadores de inadimplencia",
  subtitulo = paste0(
    "1T2018-2T2026 | Frequencia trimestral | ",
    "Meta Selic = media do trimestre"
  ),
  arquivo = "heatmap_principal_inadimplencia_2018_2026.png",
  minimo_n = 5,
  largura = 9,
  altura = 8,
  tamanho_texto = 3.6
...
# ------------------------------------------------------------
# 23.10 HEATMAP 2
# Selic + modalidades BCB + NPL 90 dias Itau
# ------------------------------------------------------------

base_heatmap_2 <- purrr::reduce(
  list(
    selic_heatmap_trimestral,
    modalidades_heatmap_trimestral,
    npl_itau_heatmap
  ),
  full_join,
  by = "Data_Trimestre"
...
resultado_heatmap_2 <- calcular_correlacao_com_n(
  base = base_heatmap_2,
  minimo_n = 5
)
...
nomes_heatmap_2 <- c(
  ...
  "Inadimplencia_Nao_Consignado_Com_Garantia" =
    "Nao consignado c/ garantia",
  
  "Inadimplencia_Nao_Consignado_Sem_Garantia" =
    "Nao consignado s/ garantia",
  ...
  "Itau_NPL90_MPME_Brasil_pct" =
    "Itau NPL MPME Brasil",
  
  "Itau_NPL90_Grandes_Empresas_Brasil_pct" =
    "Itau NPL Grandes Empresas"
)


criar_heatmap_correlacao(
  resultado_cor = resultado_heatmap_2,
  nomes_exibicao = nomes_heatmap_2,
  titulo = "Correlacao: Selic, modalidades de inadimplencia e NPL do Itau",
  subtitulo = paste0(
    "1T2018-2T2026 | Frequencia trimestral | ",
    "cinza = menos de 5 pares de observacoes"
  ),
  arquivo = "heatmap_modalidades_itau_2018_2026.png",
  minimo_n = 5,
  largura = 16,
  altura = 14,
  tamanho_texto = 2.3
...
# ------------------------------------------------------------
# 23.11 Exportacao das bases e matrizes
# ------------------------------------------------------------
...
readr::write_csv(
  ...
  file.path(
    pasta_saida,
    "base_heatmap_principal_2018_2026.csv"
  ),
  na = ""
)

write.csv(
  resultado_heatmap_1$cor_bruta,
  file = file.path(
    pasta_saida,
    "matriz_correlacao_heatmap_principal_2018_2026.csv"
  ),
  row.names = TRUE
)

write.csv(
  resultado_heatmap_1$n,
  file = file.path(
    pasta_saida,
    "matriz_n_heatmap_principal_2018_2026.csv"
  ),
  row.names = TRUE
)
...
readr::write_csv(
  ...
  file.path(
    pasta_saida,
    "base_heatmap_modalidades_itau_2018_2026.csv"
  ),
  na = ""
)

write.csv(
  resultado_heatmap_2$cor_bruta,
  file = file.path(
    pasta_saida,
    "matriz_correlacao_heatmap_modalidades_itau_2018_2026_BRUTA.csv"
  ),
  row.names = TRUE
)

write.csv(
  resultado_heatmap_2$cor_filtrada,
  file = file.path(
    pasta_saida,
    "matriz_correlacao_heatmap_modalidades_itau_2018_2026_n_min_5.csv"
  ),
  row.names = TRUE
)

write.csv(
  resultado_heatmap_2$n,
  file = file.path(
    pasta_saida,
    "matriz_n_heatmap_modalidades_itau_2018_2026.csv"
  ),
  row.names = TRUE
)


# Excel com as duas analises
...
writexl::write_xlsx(
  ...
  path = file.path(
    pasta_saida,
    "heatmaps_correlacoes_2018_2026.xlsx"
  )
)


# ------------------------------------------------------------
# 23.12 Diagnosticos no console
# ------------------------------------------------------------

cat("\n========================================\n")
cat("HEATMAP 1 - PERIODO E N\n")
cat("========================================\n")
...
cat("\n========================================\n")
cat("HEATMAP 2 - MENOR N DE PARES\n")
cat("========================================\n")
...
cat(
  paste0(
    "\nATENCAO: as series de nao consignado com e sem garantia ",
    "comecam apenas em nov/2025 no arquivo BCB fornecido. ",
    "Por isso, suas correlacoes trimestrais possuem somente ",
    "3 pares e aparecem em cinza no heatmap filtrado.\n"
  )
)
